(function (root, factory) {
  ...
  function renderInspectionReportHtml(model) {
    ...
    return `<!doctype html>
<html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Rapport de contrôles — ${escapeHtml(safeModel.chantierName)}</title>
<style>
...
</style></head><body><main class="report"><header><div><div class="brand">RailOps</div><h1>Rapport de contrôles</h1><div class="meta">${escapeHtml(safeModel.chantierName)} · ${escapeHtml(periodLabel(safeModel.period))}</div></div></header>
<section><h2>Synthèse</h2><div class="summary"><div class="card"><div class="label">Contrôles</div><div class="value">${escapeHtml(safeModel.summary?.totalControls ?? 0)}</div></div><div class="card"><div class="label">Anomalies</div><div class="value">${escapeHtml(safeModel.summary?.anomalies ?? 0)}</div></div><div class="card"><div class="label">Période</div><div class="text">${escapeHtml(periodLabel(safeModel.period))}</div></div></div></section>
<section><h2>Anomalies prioritaires</h2><table><thead><tr><th>Date</th><th>Matériel</th><th>Agent</th><th>Statut</th><th>Commentaire</th></tr></thead><tbody>${anomalyRows}</tbody></table></section>
<section><h2>Détail des contrôles</h2><table><thead><tr><th>Date</th><th>Matériel</th><th>Agent</th><th>Statut</th><th>Commentaire</th></tr></thead><tbody>${controlRows}</tbody></table></section>
<section><h2>Suivi par agent</h2><table><thead><tr><th>Agent</th><th class="num">Contrôles</th><th class="num">Anomalies</th></tr></thead><tbody>${agentRows}</tbody></table></section>
<footer>Rapport généré depuis les données RailOps fournies à l’export. Aucun indicateur n’est extrapolé.</footer></main></body></html>`;
  }
