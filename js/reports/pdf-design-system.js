(function (root, factory) {
  ...
  function renderSummaryCards(cards) {
    ...
    return `<section><div class="summary">${items.map(card => {
  ...
  function renderDocument({ title = 'Document', subtitle = '', context = '', summaryCards = [], sections = [], footer = '' } = {}) {
    return `<!doctype html>
...
</style></head><body><main class="report"><header><div><div class="brand">RailOps</div><h1>${escapeHtml(title)}</h1>${subtitle ? `<div class="meta">${escapeHtml(subtitle)}</div>` : ''}${context ? `<div class="meta">${escapeHtml(context)}</div>` : ''}</div></header>${renderSummaryCards(summaryCards)}${renderSections(sections)}<footer>${escapeHtml(footer)}</footer></main></body></html>`;
  }
