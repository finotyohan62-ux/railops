...
## 2026-08-27 — contrat documentaire de revue de dérive 22:13 Europe/Paris

- État réel contrôlé avant changement : branche `security/v150b2b-rls-ready` au head initial `ca48b52bd7fd7952b9990ca916c86412684176b4`; `main` observé et laissé intact à `37b216936a6692d54f82cbc004b30c936d13785a`, divergence contrôlée `ahead=492`, `behind=2`, merge-base `20f7e028ac5e3d0ac401d41ec3561af09e252694`. Les deux commits propres à `main` touchent `js/core/sync.js` et `tests/sync-error-handling.test.js`; aucune intégration automatique n'a été effectuée.
- Contrôle Supabase frais et strictement en lecture seule : RLS reste activé mais non forcé sur `chantiers`, `materiels`, `scans` et `users`, avec respectivement 4/4/4/2 policies ; aucune donnée, policy ou configuration n'a été modifiée.
- Amélioration réversible : ajout de `tests/v150b2b-branch-drift-doc-contract.test.js`, auto-découvert par le runner, qui garantit que la documentation de revue de dérive conserve les contrôles de SHA `main`, SHA de branche, merge-base, vérification Supabase en lecture seule, interdiction d'intégration automatique et maintien de `main` intact. Commit principal : `c74cbaf41a8383a09936be2e2a839b2fc879399e` (`test: guard branch drift review safety contract`).
...
- État réel contrôlé avant changement : branche `security/v150b2b-rls-ready` au head `4cf16b4112b993e40bfabf60d803e4479503bd7e`; `main` observé et laissé intact à `37b216936a6692d54f82cbc004b30c936d13785a`, divergence `ahead=502`, `behind=2`, merge-base `20f7e028ac5e3d0ac401d41ec3561af09e252694`. Les fichiers propres à `main` à impact runtime/régression sont `js/core/sync.js` et `tests/sync-error-handling.test.js` ; aucune intégration n'a été effectuée.
- Contrôle Supabase strictement en lecture seule : projet `railops` `ACTIVE_HEALTHY`; RLS activé et non forcé sur `chantiers`, `materiels`, `scans` et `users`, avec respectivement 4/4/4/2 policies.
...
## 2026-08-28 — contrat de checklist de maintenance sûre 04:17 Europe/Paris

- État réel contrôlé avant changement : branche `security/v150b2b-rls-ready` au head initial `ceed0d30b1e42ec5fa558c8ca90cf0a14f8eec12`; `main` observé et laissé intact à `37b216936a6692d54f82cbc004b30c936d13785a`, divergence `ahead=507`, `behind=2`, merge-base `20f7e028ac5e3d0ac401d41ec3561af09e252694`. Les fichiers propres à `main` à impact runtime/régression restent `js/core/sync.js` et `tests/sync-error-handling.test.js`; aucune intégration n'a été effectuée.
- Contrôle Supabase strictement en lecture seule : projet `railops` `ACTIVE_HEALTHY`; RLS activé et non forcé sur `chantiers`, `materiels`, `scans` et `users`, avec respectivement 4/4/4/2 policies.
- Diagnostic : la checklist `docs/v150b2b-safe-change-checklist.md` portait déjà les interdictions de merge, déploiement, mutation Supabase et changement métier, mais aucun test ne protégeait ces garde-fous documentaires contre une dérive future.
- Amélioration réversible : ajout de `tests/v150b2b-safe-change-checklist-contract.test.js`, auto-découvert par le runner, qui vérifie 14 marqueurs de sûreté et la règle d'arrêt en cas d'ambiguïté. Commit principal : `a98e17d4e4e636ba0081e3128831714d6c2b46e8` (`test: guard safe maintenance checklist contract`).
- Vérification fraîche : `v150B-2B checks` runs push `33135508015` et PR `33135509711` sont `success`; `RailOps lifecycle regression` `33135509707`, `RailOps modules regression` `33135509745` et `Final RLS hotfix check` `33135509720` sont également `success`.
- Garde-fous respectés : aucun code runtime, workflow, règle métier, permission applicative, donnée, migration, schéma, RLS stricte, Import, logique Multi-chantier ou purge hebdomadaire modifiés ; aucun merge, rebase, cherry-pick ni déploiement production effectué ; `main` n'a pas été modifié.
- Point en attente : aucun choix produit, test utilisateur ou changement risqué requis ; les changements sécurité/data et toute intégration de la dérive de `main` restent volontairement hors scope sans validation explicite.
