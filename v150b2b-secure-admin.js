(function(){
'use strict';
const VERSION='150B2B-secure-admin-2';
...
async function refresh(){try{if(S?.__ownerAdminMode&&window.RailOpsOwnerMode150B2B?.reloadAdmin){await window.RailOpsOwnerMode150B2B.reloadAdmin(false);if(typeof render==='function')render();return;}if(window.RailOpsSecurity150B2B?.reload){await window.RailOpsSecurity150B2B.reload();if(typeof render==='function')render();}}catch(e){console.warn('[RailOps v150B-2B] refresh',e);}}
...
function explicitAdmin(){return !!S?.isAdminOwner&&!!S?.__ownerAdminMode&&S?.role==='admin';}
...
saveUser=async function(id){
  ...
  if(!explicitAdmin()){msg('Passez en mode Administration pour gérer les comptes.','danger');return;}
  try{
...
doChangePass=async function(id,selfMode){
  ...
  if(!selfMode&&!explicitAdmin()){msg('Passez en mode Administration pour modifier un autre compte.','danger');return;}
  try{
